...
(********************************************)
subsection{*Closed forms for Ackermann 1 and Ackermann 2, from Cormen et al.*}
(********************************************)
...
lemma compow_Ackermann_1_eq_lemma: assumes "i>0" "x>0" shows "Suc (2 * (2 ^ i * Suc x - 1)) = 2 ^ Suc i * Suc x - 1"
proof -
  {
    have sg1: "2 ^ Suc i * Suc x > 1"
      using assms
      by (metis One_nat_def Suc_mono less_1_mult one_less_numeral_iff power_gt1 semiring_norm(76))
  } note sg1 = this
  show ?thesis
   ...
   show ?case 
     ...
     subgoal
       ...
       subgoal apply auto
         ...
         apply (subst diff_mult_distrib2)
         apply auto
         by (metis One_nat_def Suc_1 Suc_diff_Suc one_less_numeral_iff power_gt1_lemma semiring_norm(76))
       done
     apply (cases "x > 0")
     by auto
qed
...
proof -
  {have "2 = (Suc 1)" by simp} note sg1=this
  show ?thesis
    apply (subst sg1)
...
(********************************************)
subsection {*Every Ackermann k is inflationary. That is, x \<le> Ackermann k x holds.*}
(********************************************)

abbreviation inflationary where "inflationary f \<equiv> \<forall>x. x \<le> f x"
(*There is probably an Isabelle thing for this, didn't find it*)
...
(********************************************)
subsection {*astep is inflationary. That is, for inflationary f, f x \<le> astep f x holds*}
(********************************************)
...
(********************************************)
subsection {*Ackermann is monotonic in its first argument. The results, which are functions, 
...
(********************************************)
subsection {*Power can be expressed with compow*}
(********************************************)
...
subsection {*A slightly more powerful version of the previous lemma. It is not a
   direct consequence of the previous lemma, because 2 ^ (log n) is
...
proof -
  ...
  have sg2: "\<forall> x y. x\<le> y \<longrightarrow> Ackermann 2 x \<le> Ackermann 2 y" using mono_Ackermann[of 2] unfolding mono_def .
...
(********************************************)
subsection {*Some stuff about asymptotics*}
(********************************************)
...
(********************************************)
subsection {*Every Ackermann k is strictly inflationary. That is, x < Ackermann k x holds.*}
(********************************************)
...
(********************************************)
subsection {*For every n other than zero, compow n (Ackermann k) is strictly inflationary.*}
(********************************************)
...
(********************************************)
subsection {*The function  \<lambda>k. Ackermann k x is strictly monotonic in k.*}
(********************************************)
...
(********************************************)
subsection {*Some stuff about orbits of inflationary functions*}
(********************************************)
...
proof -
  {
    ...
    have "x + i \<le> compow i f x" using assms proof (induction i)
      ...
      then show ?case 
        ...
        case 1
        have "Suc (x + i) \<le> f ((f ^^ i) x)" using assms Suc(1)[OF \<open> \<forall>x. x < f x\<close>]
          using Suc_leI le_less_trans by blast
...
(********************************************)
subsection {* (\<lambda> i. compow i (Ackermann k) x) is monotonic.*}
(********************************************)
...
interpretation A1nn: f_nat_nat "\<lambda>k. Ackermann k 1"
  ...
  using Ackermann_k_x_tends_to_infinity_along_k[of 1] by simp

definition \<alpha>' where
  "\<alpha>' \<equiv> A1nn.\<alpha>\<^sub>f "

lemma \<alpha>_alt_eq: "\<alpha> = \<alpha>'" unfolding \<alpha>_def \<alpha>'_def A1nn.\<alpha>\<^sub>f_def by blast


lemma mono_\<alpha>: "mono \<alpha>"
  apply (subst \<alpha>_alt_eq) unfolding \<alpha>'_def
...
(* "\<alpha> n" is at most "Suc (alpha (log n))". This gives a weak sense of how
...
(* This property also shows that "\<alpha> n" and "\<alpha> (log n)" are
   ...
   find, even though they could easily obtain "\<alpha> (log n)". See Exercise
...
lemma \<alpha>_n_0_\<alpha>_logn:
  ...
  subgoal using two_le_\<alpha> apply (subst (asm) \<alpha>_alt_eq) unfolding \<alpha>'_def
  proof goal_cases
    case 1
    have "n \<le> Ackermann 2 (Discrete.log n)" using Ackermann_2_log_lower_bound[of n] .
    also have "\<dots> \<le> Ackermann 2 (Ackermann (A1nn.\<alpha>\<^sub>f (Discrete.log n)) 1)" 
      using A1nn.f_\<alpha>\<^sub>f[of "Discrete.log n"] mono_Ackermann unfolding mono_def by blast
      also have "\<dots> \<le> Ackermann (A1nn.\<alpha>\<^sub>f (Discrete.log n))
          (Ackermann (A1nn.\<alpha>\<^sub>f (Discrete.log n)) 1)" using 1(1) 
          mono_Ackermann_in_k[of "(Ackermann (A1nn.\<alpha>\<^sub>f (Discrete.log n)) 1)"] 
        unfolding mono_def by blast
      finally show ?case .
  qed
  subgoal  proof goal_cases
    case 1
    have 2: "4 \<le> Discrete.log n " using assms by (simp add: prove_le_log)
    have "2 \<le> A1nn.\<alpha>\<^sub>f (Discrete.log n)" using two_le_\<alpha>[OF 2] 
      apply (subst (asm) \<alpha>_alt_eq) unfolding \<alpha>'_def .
...
(* Alstrup et al. define the function alphar as follows (page 17).
   Note that their definition of Ackermann is not the same as ours -- they
...
lemma alphar_one: assumes "r = 1" shows "alphar n  = Suc (\<alpha> (Suc n))"
  apply (subst alphar_alt_eq)
  apply (subst \<alpha>_alt_eq)
  unfolding alphar'_def prealphar_def \<alpha>'_def pnn.\<alpha>\<^sub>f_def A1nn.\<alpha>\<^sub>f_def using assms by blast
...
(* "Ackermann (prealphar n) r" is greater than n (page 17). *)
